...
4. **Building & vegetation data:** the Overpass API (via a caching Cloudflare Worker, `overpass-cache/`, 30-day KV TTL) fetches all building footprints in the route bounding box, with real heights from OSM tags (`height`, `building:levels`), plus trees (`natural=tree`, `natural=tree_row`) with seasonal leaf-coverage modelling (deciduous vs evergreen, based on the chosen date and hemisphere).
